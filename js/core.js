...
//[GTR]
		vkontakte: function(val)
		{
			return /^http:\/\/vk\.com\/[^\/].*$/i.test(val) ? true : (val == '') ? false : fn_show_social_notification_error('vk');
		},
		twitter: function(val)
		{
			return /^http:\/\/twitter\.com\/#!\/[a-zA-Z0-9_]{1,15}/i.test(val) ? true : (val == '') ? false : fn_show_social_notification_error('twitter');
		},
		facebook: function(val)
		{
			//return /^(https?:\/\/)?((w{3}\.)?)facebook\.com\/(?:[^\s()\\\[\]{};:'",<>?«»“”‘’]){5,}$/i.test(val) ? true : false;
			return /^http:\/\/(www|[a-zA-Z]{2}-[a-zA-Z]{2})\.facebook\.com\/(people\/[a-zA-Z0-9-]+\/[0-9]+|[a-zA-Z0-9-\.]+)[\/]?$/i.test(val) ? true : (val == '') ? false : fn_show_social_notification_error('facebook');
		},
		/*username: function(val)
		{
			return /^[А-Я]+[а-я]* [А-Я]+[а-я]*$/i.test(val) ? true : false;
		},*/
//[/GTR]
		blank: function(val)
		{
...
//[GTR]
			// Check the twitter field
			if (lbl.hasClass('cm-twitter')) {
				if (jQuery.is.twitter(elm.val()) == false) {
					if (lbl.hasClass('cm-required') || jQuery.is.blank(elm.val()) == false) {
						this.form_message(lang.error_validator_twitter, lbl);
						is_ok = false;
						set_mark = true;
					}
				}
			}
			if (lbl.hasClass('cm-facebook')) {
				if (jQuery.is.facebook(elm.val()) == false) {
					if (lbl.hasClass('cm-required') || jQuery.is.blank(elm.val()) == false) {
						this.form_message(lang.error_validator_facebook, lbl);
						is_ok = false;
						set_mark = true;
					}
				}
			}
			if (lbl.hasClass('cm-vkontakte')) {
				if (jQuery.is.vkontakte(elm.val()) == false) {
					if (lbl.hasClass('cm-required') || jQuery.is.blank(elm.val()) == false) {
						this.form_message(lang.error_validator_vkontakte, lbl);
						is_ok = false;
						set_mark = true;
					}
				}
			...
			/*
			if (lbl.hasClass('cm-username')) {
				if (jQuery.is.username(elm.val()) == false) {
					if (lbl.hasClass('cm-required') || jQuery.is.blank(elm.val()) == false) {
						this.form_message(lang.error_validator_username, lbl);
						is_ok = false;
						set_mark = true;
					}
				}
...
//[GTR]
function fn_show_social_notification_error(data)
{
	if ( $('.cm-notification-container').find('div') ) {
		jQuery.processNotifications();
	}
	jQuery.showNotifications({'data': {'type': 'E', 'title': lang.error, 'message': lang['text_'+data+'_error'], 'save_state': false}});
	return false;
}
//[/GTR]
